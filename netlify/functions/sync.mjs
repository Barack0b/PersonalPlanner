...
export default async (req) => {
  try {
    ...
    try {
      ...
      return json(
        { error: 'Could not connect to storage: ' + (err && err.message ? err.message : String(err)) },
        500
      );
    }
    ...
    // Last-resort catch-all: never let an unhandled error produce Netlify's
    // raw crash page. Always return clean JSON so the app can react to it
    // instead of showing you a dead end.
    return json(
